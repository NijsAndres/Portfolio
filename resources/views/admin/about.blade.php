@extends('admin.layouts.app')

@section('title', 'About Section')

@section('content')
    <form method="POST" action="{{ route('admin.about.update') }}" class="max-w-2xl space-y-6">
        @csrf
        @method('PUT')

        <div class="bg-white rounded-lg border border-gray-200 p-6 space-y-5">
            <div>
                <label for="bio_text" class="block text-sm font-medium text-gray-700 mb-1">Bio</label>
                <textarea id="bio_text" name="bio_text" rows="6"
                          class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ old('bio_text', $about->bio_text) }}</textarea>
            </div>

            <div>
                <label for="born_in" class="block text-sm font-medium text-gray-700 mb-1">Born in</label>
                <input type="text" id="born_in" name="born_in"
                       value="{{ old('born_in', $about->born_in) }}"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            </div>

            <div>
                <label for="languages" class="block text-sm font-medium text-gray-700 mb-1">Languages</label>
                <input type="text" id="languages" name="languages"
                       value="{{ old('languages', $about->languages) }}"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            </div>

            <div>
                <label for="date_of_birth" class="block text-sm font-medium text-gray-700 mb-1">Date of birth</label>
                <input type="text" id="date_of_birth" name="date_of_birth"
                       value="{{ old('date_of_birth', $about->date_of_birth) }}"
                       placeholder="21/04/2003"
                       class="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                <p class="mt-1 text-xs text-gray-400">Stored as free text (e.g. 21/04/2003).</p>
            </div>
        </div>

        <div class="flex justify-end">
            <button type="submit" class="bg-indigo-600 text-white text-sm font-medium px-5 py-2 rounded-md hover:bg-indigo-700 transition">
                Save changes
            </button>
        </div>
    </form>
@endsection
